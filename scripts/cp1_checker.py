...
if len(sys.argv) < 7:
    sys.stderr.write(
        'Usage: %s <ip> <port> <#trials> <#writes and reads per trial> <max # bytes to write at a time> <#connections> \n' % (sys.argv[0]))
    sys.exit(1)
...
for i in xrange(numTrials):
    ...
    for j in xrange(numWritesReads):
        ...
        print "{} Send {} bytes".format(j, random_len)

    for j in xrange(numWritesReads):
        data = socketSubset[j].recv(randomLen[j])

        if(len(data) != len(randomData[j]) ):
            print "{} is wrong: data: {}, rand: {}".format(j, len(data), len(randomData[j]))
            sys.stderr.write(
                "Error: Data received is not the same as sent! \n")
            sys.exit(1)
